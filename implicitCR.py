...
2) Skip training (split -> post-eval):
   python implicitCR.py --skip-train

4) Use different dataset CSVs:
...
class RunConfig:
    ...
    removal_smoothing_lambda: int = 4
    lambda_truncate_length: int = 100
    remove_per_epoch: int = 2
    reset_optimizer: bool = True


def parse_args() -> RunConfig:
    parser = argparse.ArgumentParser(
        description="Run implicit-CR causal relation fine-tuning pipeline (split -> train -> post-eval)."
    )
    ...
    parser.add_argument("--removal-smoothing-lambda", type=int, default=4)
    parser.add_argument("--lambda-truncate-length", type=int, default=100)
    parser.add_argument("--remove-per-epoch", type=int, default=2)
    ...
    return RunConfig(
        ...
        removal_smoothing_lambda=args.removal_smoothing_lambda,
        lambda_truncate_length=args.lambda_truncate_length,
        remove_per_epoch=args.remove_per_epoch,
...
def infer_dataset_type(cfg: RunConfig) -> str:
    dataset_hint = f"{cfg.source_csv} {cfg.train_split_csv} {cfg.test_split_csv}".lower()
    if "collider" in dataset_hint:
        return "collider"
    if "chain" in dataset_hint:
        return "chain"
    return "confounder"


def build_cot_text(example: Dict[str, Any], dataset_type: str, noisy_prefix: bool) -> str:
    cot = ""
    ...
    return sed_assoc + sed_warn + cot


def make_processing_funcs(tokenizer: Any, cfg: RunConfig):
    dataset_type = infer_dataset_type(cfg)

    def processing_func(example, scheduled_to_remove: int, offset: int):
        ...
        cot_text = build_cot_text(example, dataset_type, cfg.noisy_prefix)
        target_text = "<FINAL_ANSWER> " + example["label"]

        to_remove = scheduled_to_remove + offset
        cot_ids = tokenizer(cot_text, add_special_tokens=False)["input_ids"]
        if to_remove < 0:
            to_remove = 0
        if to_remove > len(cot_ids):
            to_remove = len(cot_ids)

        cot_tail_ids = cot_ids[to_remove:]
        cot_tail_text = tokenizer.decode(cot_tail_ids, skip_special_tokens=True)
        y_text = cot_tail_text + target_text
        full_text = prompt_text + y_text
...
def build_eval_dataset(test_df: pd.DataFrame, eval_processing_func):
    ...
    return test_dataset, eval_gold, eval_texts
...
def build_lambda_dist(removal_smoothing_lambda: float, truncate_length: int = 100) -> np.ndarray:
    if removal_smoothing_lambda == float("inf"):
        lambda_distribution = np.zeros(truncate_length, dtype=np.float64)
        lambda_distribution[0] = 1.0
        return lambda_distribution

    lam = float(removal_smoothing_lambda)
    positions = np.arange(truncate_length, dtype=np.float64)
    lambda_distribution = (1.0 - np.exp(-lam)) * np.exp(-lam * positions)
    lambda_distribution[-1] += (1.0 - lambda_distribution.sum())
    return lambda_distribution


class ImplicitCRDataset(Dataset):
    def __init__(self, df: pd.DataFrame, processing_func, lambda_dist: np.ndarray):
    ...
    def _sample_offset(self) -> int:
        length = self.lambda_dist.shape[0]
        return int(np.random.choice(np.arange(length), p=self.lambda_dist))

    def __getitem__(self, idx: int):
        ex = self.df.iloc[idx]
        scheduled_offset = self._sample_offset()
        return self.processing_func(
            ex,
            scheduled_to_remove=self.scheduled_to_remove,
            offset=scheduled_offset,
        )


class StepwiseRemovalSchedule(TrainerCallback):
    def __init__(self, train_dataset: ImplicitCRDataset, remove_per_epoch: int = 8, reset_optimizer: bool = True):
        self.ds = train_dataset
        self.delta = int(remove_per_epoch)
        self.reset_optimizer = bool(reset_optimizer)
        self.steps_per_epoch = None
        self.k_per_token = None
        self.prev_s = -1

    def _init_from_dataloader(self, trainer, args):
        raw_batches = len(trainer.get_train_dataloader())
        accum = int(getattr(args, "gradient_accumulation_steps", 1))
        self.steps_per_epoch = math.ceil(raw_batches / accum)
        self.k_per_token = int(round(self.steps_per_epoch / self.delta))

    def on_train_begin(self, args, state, control, **kwargs):
        trainer = kwargs.get("trainer", None)
        if trainer is not None:
            self._init_from_dataloader(trainer, args)

    def on_step_begin(self, args, state, control, **kwargs):
        if self.steps_per_epoch is None or self.k_per_token is None:
            trainer = kwargs.get("trainer", None)
            if trainer is None:
                return
            self._init_from_dataloader(trainer, args)
        ...
        total_steps_per_epoch = self.steps_per_epoch
        i = t % total_steps_per_epoch
        s_value = int(t // total_steps_per_epoch * self.delta + min(self.delta, i // self.k_per_token))
        self.ds.scheduled_to_remove = s_value

        if self.reset_optimizer and s_value > self.prev_s and self.prev_s >= 0:
            trainer = kwargs.get("trainer", None)
            if trainer is not None:
                trainer.create_optimizer()
                trainer.create_scheduler(num_training_steps=trainer.args.max_steps)
...
def run_training(
    ...
    lambda_dist = build_lambda_dist(cfg.removal_smoothing_lambda, truncate_length=cfg.lambda_truncate_length)
    train_dataset = ImplicitCRDataset(train_df, processing_func, lambda_dist=lambda_dist)
    ...
    trainer.add_callback(
        StepwiseRemovalSchedule(
            train_dataset,
            remove_per_epoch=cfg.remove_per_epoch,
...
def main() -> None:
    ...
    test_dataset, eval_gold, eval_texts = build_eval_dataset(
        test_df=test_df,
