"""
How to run:
1) Full pipeline (split -> pre-eval -> train -> post-eval): default dataset is confounder
   python noCoT.py

2) Skip training (split -> pre-eval -> post-eval):
   python noCoT.py --skip-train

3) Use a different dataset CSV:
...
def parse_args() -> RunConfig:
    parser = argparse.ArgumentParser(
        description="Run no-CoT causal relation fine-tuning pipeline (split -> pre-eval -> train -> post-eval)."
    )
    ...
    parser.add_argument("--pre-eval-batch-size", type=int, default=32)
    ...
    return RunConfig(
        ...
        pre_eval_batch_size=args.pre_eval_batch_size,
...
def load_main_model_and_tokenizer(
    cfg: RunConfig,
) -> Tuple[Any, Any]:
    ...
    return [
        {"role": "system", "content": SYSTEM_PROMPT},
        {
            ...
            "content": (
                ...
                "When you are ready to give your final answer, output exactly one line that begins with the token <FINAL_ANSWER> "
                "followed by exactly one token: Yes or No, indicating whether the causal relationship described in the question exists.\n"
...
def make_processing_funcs(
    ...
    def processing_func(example):
        ...
        full_len = int(sum(tokenized_full["attention_mask"]))
...
def _find_subseq(hay: Sequence[int], needle: Sequence[int]) -> int:
    if not needle or len(needle) > len(hay):
        return -1
    hay_len = len(hay)
    needle_len = len(needle)
    for i in range(hay_len - needle_len + 1):
        if hay[i : i + needle_len] == list(needle):
            return i
    return -1
...
def _first_decision_after_marker(
    ...
    marker_start = _find_subseq(gen_ids, final_ids)
    if marker_start == -1:
        return None

    pos = marker_start + len(final_ids)
    if pos < len(gen_ids) and (gen_ids[pos] in yes_ids or gen_ids[pos] in no_ids):
        return gen_ids[pos]

    for k in range(1, 4):
        if pos + k < len(gen_ids) and (gen_ids[pos + k] in yes_ids or gen_ids[pos + k] in no_ids):
            return gen_ids[pos + k]
    return None


def _last_content_token_after_prompt(seq_ids: Sequence[int], prompt_len: int, stop_ids: set) -> Optional[int]:
    new_ids = list(seq_ids[prompt_len:])
    while new_ids and new_ids[-1] in stop_ids:
        new_ids = new_ids[:-1]
    return new_ids[-1] if new_ids else None


def run_pretraining_generation_eval(
    model: Any,
    tokenizer: Any,
    test_df: pd.DataFrame,
    yes_first_ids: set,
    ...
    final_ids = tokenizer.encode(final_tag, add_special_tokens=False) if use_final_marker else []
    ...
    prompts = [
        ...
        for q in test_df["data"].tolist()
    ]
    gold = np.array([1 if _norm_label(v) else 0 for v in test_df["label"].tolist()], dtype=np.int64)

    preds: List[np.ndarray] = []
    normal_count = 0
    with torch.inference_mode():
        for start in tqdm(range(0, len(prompts), cfg.pre_eval_batch_size), desc="Pre-train eval (gen-style)"):
            batch_prompts = prompts[start : start + cfg.pre_eval_batch_size]
            enc = tokenizer(
                batch_prompts,
                return_tensors="pt",
                padding=True,
            ...
            for i in range(gen.sequences.size(0)):
                ...
                #print(tokenizer.batch_decode(gen.sequences, skip_special_tokens=True))
                tok = None
                if use_final_marker:
                    tok = _first_decision_after_marker(seq, lp, final_ids, yes_first_ids, no_first_ids, stop_ids)
                if tok is None:
                    tok = _last_content_token_after_prompt(seq, lp, stop_ids)

                if tok in yes_first_ids:
                    batch_pred.append(1)
                    normal_count += 1
            ...
            preds.append(np.array(batch_pred, dtype=np.int64))

    preds_cat = np.concatenate(preds, axis=0)
    assert preds_cat.shape[0] == gold.shape[0]
    acc = float((preds_cat == gold).mean())
    num_correct = int((preds_cat == gold).sum())
    num_total = int(gold.shape[0])

    print("=== Pre-training Evaluation (Generation Style) ===")
    print(f"Samples: {num_total}")
    print(f"Accuracy: {acc:.4f}  (Correct: {num_correct} / {num_total})")
    print(f"Normal_ans_count: {normal_count}")
    return {
        "accuracy": acc,
        "correct": num_correct,
        "total": num_total,
        "normal_ans_count": normal_count,
    }
...
def run_generation_eval(
    ...
    stop_ids = {tid for tid in [tokenizer.eos_token_id, eot_id] if tid is not None}
    final_tag = "<FINAL_ANSWER>"
    final_ids = tokenizer.encode(final_tag, add_special_tokens=False)
...
def main() -> None:
    ...
    run_pretraining_generation_eval(
        model=model,
        tokenizer=tokenizer,
        test_df=test_df,
        yes_first_ids=yes_first_ids,
